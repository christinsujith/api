...
class OAuthToken {/*{{{*/
   ...
   */
  function to_string() {/*{{{*/
    return "oauth_token=" . rawurlencode($this->key) . 
        "&oauth_token_secret=" . rawurlencode($this->secret);
  }/*}}}*/
...
class OAuthSignatureMethod_HMAC_SHA1 extends OAuthSignatureMethod {/*{{{*/
  ...
  public function build_signature($request, $consumer, $token) {/*{{{*/
    $sig = array(
      ...
      preg_replace('/%7E/', '~', rawurlencode($request->get_normalized_http_url())),
...
class OAuthSignatureMethod_PLAINTEXT extends OAuthSignatureMethod {/*{{{*/
  ...
  public function build_signature($request, $consumer, $token) {/*{{{*/
    ...
    if ($token) {
      array_push($sig, rawurlencode($token->secret));
    } else {
      array_push($sig, '');
    }
...
class OAuthRequest {/*{{{*/
  ...
  public function to_postdata() {/*{{{*/
    ...
    foreach ($this->parameters as $k => $v) {
      //if ($k == "oauth_token" || $k == "oauth_consumer_key") {
      //  $total[] = rawurlencode($k) . "=" . $v;
      //} else {
        $total[] = rawurlencode($k) . "=" . rawurlencode($v);
      //}
    }
